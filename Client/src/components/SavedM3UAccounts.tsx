...
const SavedM3UAccounts = () => {
  ...
  const [editForm, setEditForm] = useState<M3UCredentials | null>(null);
  ...
  const loadAccounts = () => {
    const saved = getSavedM3UAccounts();
    setAccounts(saved);
  };

  const handleSelectAccount = async (account: M3UCredentials) => {
    setIsLoading(true);
    setError('');

    try {
      // Playlist'i yükle
      const response = await loadM3UPlaylist(account);
      
      if (response.success && response.channels) {
        // Başarılı - aktif hesabı ayarla
        setActiveM3UAccount(account, response.channels);
  ...
  const handleEditAccount = (account: M3UCredentials) => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      // Playlist'i test et
      const response = await loadM3UPlaylist(editForm);
      
      if (response.success) {
        // Başarılı - güncelle
        const { getSavedM3UAccounts, saveM3UAccount } = await import('../services/m3uService');
        const updatedAccount = {
          ...editForm,
          id: editingId,
          channelCount: response.channels?.length
        };
        saveM3UAccount(updatedAccount);
        
        // Backend'e gönder (kayıtsız kullanıcı için)
        const { sendM3UAccount } = await import('../services/anonymousAccountService');
        await sendM3UAccount(updatedAccount);
...
export default SavedM3UAccounts;
